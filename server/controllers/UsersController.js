...
export const create = async (req, res, next) => {
  try {
    ...
    // If avatar is present, set it as the filename
    ...
    if (avatar) {
      // Adjust the file paths using 'path' module instead of '__dirname'
      fs.renameSync(
        avatar.path,
        path.join(__dirname, "..", permanentStorage, avatar.filename)
      );
    }
...
export const update = async (req, res, next) => {
  try {
    ...
    if (avatar && fs.existsSync(avatar.path)) {
      ...
      fs.unlinkSync(avatar.path);
      await new Promise((resolve) => setTimeout(resolve, 100));
      if (fs.existsSync(`${permanentStorage}/${user.avatar}`))
